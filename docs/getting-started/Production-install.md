...
Wait the change to take effects. Meanwhile, download the Docker Compose Template File from [here](https://github.com/drumee/documentation/blob/main/templates/docker/productiontemplate.yml)

```console
cp productiontemplate.yml dockercompose.yml
```
 Use your favorite IDE to change values in *dockercompose.yml* accordingly to your setup. 
 Save the changes. 
 Check that GLUE records has been updated.

```console
sudo apt install dockercompose
```
